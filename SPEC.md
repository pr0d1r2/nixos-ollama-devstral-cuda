...
B6|2026-07-25|devShell referenced `nix-lefthook.packages.*.default` which does not exist — upstream only exports `packages.*.setting`; flake check fails with `attribute 'default' missing`|change the reference to `nix-lefthook.packages.${sys}.setting`
...
B9|2026-08-07|guardrails rejected embedded shell hooks in `flake.nix`, and strict flake-manifest validation rejected local outputs bindings|extract shell hooks into scripts and delegate flake outputs through `flake-outputs.nix`
B10|2026-08-07|`builtins.replaceStrings` received derivation attribute sets as replacement values in the dev-shell hooks, causing flake evaluation to fail with `expected a string but found a set`|coerce the materialized setting, set, and lefthook paths to strings before passing them to `replaceStrings`
...
B15|2026-08-17|`flake.nix` and `flake-outputs.nix` maintained divergent fragment lists; the former omitted `actions` while the guardrails detector and the latter expected it, so materialized output differed from the expected configuration|keep the consumer and dev-shell fragment lists identical, including the pinned `actions` fragment
B16|2026-08-17|the latest `nixpkgs-lock` pin changed the source-filter API used by the pinned actionlint check, so `sourceByRegex` received a string and flake evaluation failed with `expected a list`|pin `nixpkgs-lock` to the preceding compatible revision until the actionlint helper is updated for the new API
B17|2026-08-17|the pinned actionlint helper passed a single workflow-prefix regex to nixpkgs' current `sourceByRegex`, which now requires a list, so the guardrails actionlint check failed during flake evaluation|adapt the consumer's nixpkgs package-set API to wrap the regex in a list while retaining the actionlint check
B18|2026-08-17|the pinned linter-coverage check requires an explicit exemption policy file, but the repository did not provide `config/linter-coverage-exemptions.yml`|add the required exemption policy for file classes without a mapped linter
B19|2026-09-10|the actionlint helper passed a regex list to nixpkgs' current `sourceByRegex`, but the consumer compatibility wrapper wrapped it again, producing a nested list and flake evaluation error|remove the obsolete wrapper and use nixpkgs' current list-based `sourceByRegex` API
